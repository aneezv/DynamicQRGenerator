...
export const DynamicQRPage: React.FC = () => {
  ...
  const benefits = [
    { icon: LinkIcon, title: 'Short URLs', description: 'Get clean, branded short URLs for your QR codes', gradient: 'from-blue-500 to-cyan-500' },
    { icon: BarChart3, title: 'Real-time Analytics', description: 'Track scans, locations, and user behavior', gradient: 'from-green-500 to-emerald-500' },
    { icon: Shield, title: 'Always Editable', description: 'Change destination URLs without reprinting codes', gradient: 'from-purple-500 to-pink-500' },
    { icon: Zap, title: 'Instant Updates', description: 'Updates take effect immediately across all codes', gradient: 'from-orange-500 to-red-500' },
  ];
    ...
    <>
      <div className="min-h-screen bg-gradient-to-br from-primary-50 via-purple-50 to-pink-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
        <div className="px-4 py-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
          ...
          {!editId && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-1 gap-4 mb-8 md:grid-cols-2 lg:grid-cols-4">
              {benefits.map((benefit, index) => (
                <motion.div key={benefit.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + index * 0.1 }} whileHover={{ scale: 1.05 }} className="group">
                  <Card className="h-full p-6 text-center transition-all duration-300 border-0 shadow-lg bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm hover:shadow-xl">
                    ...
                    <h3 className="mb-2 font-semibold text-gray-900 dark:text-gray-100">{benefit.title}</h3>
